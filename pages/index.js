...
export default function Home() {
  ...
  useEffect(() => {
    return onSnapshot(q, (docs) => {
      ...
      console.log('docs', Array.from(docs));
      setList(newList);
    });
  }, []);
  ...
  const columns = [
            ...
            <ul>
              {record.items.map((item) => (
                <Fragment key={item.name}>
                  <li>
                    {item.name} {formatter.format(item.price)}원 &times;{' '}
                    {item.count}
                  </li>
                </Fragment>
              ))}
      ...
      render: (text, record) => {
        return (
          <div>
            합계 :{' '}
            {formatter.format(
              sum(record.items.map((item) => item.count * item.price))
            )}
          </div>
        );
      },
        ...
        return (
          <>
            <Button
              ...
              onClick={() => {
                updateDoc(doc(store, 'orders', record.id), {
                  status: '주문 완료',
                });
              }}
            >
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return DateTime.fromSeconds(record.createdAt.seconds).toFormat(
          'LL-dd HH:mm'
        );
      },
    },
